...
public class ExternalFileOperations {
      ...
      try {
        ...
        try (final FileImageOutputStream outputStream = new FileImageOutputStream(file);
            final GifSequenceWriter writer =
                new GifSequenceWriter(outputStream, imgList.get(0).getType(), 6000, false)) {
    ...
    if (file != null) {
      try {
        ...
        ArrayList<String> imageNames =
            new ArrayList<String>(); //  To save the name of the generated series of images
  ...
  public ArrayList<BufferedImage> toImages(ArrayList<SavedSlide> slideArrayList)
      throws IOException {
    ArrayList<BufferedImage> images = new ArrayList<>();
    BufferedImage endPane =
        ImageIO.read(this.getClass().getResource("/main/resources/images/end.png"));

    for (SavedSlide slide : slideArrayList) {
      BufferedImage pane =
          ImageIO.read(this.getClass().getResource("/main/resources/images/pane.png"));
      ...
      if (slide.getCharacterLeft().getScale() == -1) {
        g.drawImage(
            SwingFXUtils.fromFXImage(left.getImage(), null), 17 + 375, 342, -375, 375, null);
      } else {
      ...
      if (slide.getCharacterRight().getScale() == -1) {
        g.drawImage(
            SwingFXUtils.fromFXImage(right.getImage(), null), 402 + 375, 342, -375, 375, null);
      } else {
                ...
                this.getClass()
                    .getResource(
                        "/main/resources/images/"
                            + slide.getCharacterLeft().getBubble().getName()
                            + ".png"));
        g.drawImage(bubble, 30, 135, null);
        if (metrics.stringWidth(slide.getCharacterLeft().getText())
            <= 295) { // if text fits on 1 line
          g.drawString(
              slide.getCharacterLeft().getText(),
              200 - (metrics.stringWidth(slide.getCharacterLeft().getText()) / 2),
              250);
        } else {
          ...
          g.drawString(
              firstLine, 200 - (metrics.stringWidth(firstLine) / 2), 250 - metrics.getMaxDescent());
          g.drawString(
              secondLine,
              200 - (metrics.stringWidth(secondLine) / 2),
              250 + metrics.getMaxAscent());
        }
      }
        ...
        bubble =
            ImageIO.read(
                this.getClass()
                    .getResource(
                        "/main/resources/images/"
                            + slide.getCharacterRight().getBubble().getName()
                            + ".png"));
        g.drawImage(bubble, 435, 135, null);
        if (metrics.stringWidth(slide.getCharacterRight().getText())
            <= 295) { // if text fits on 1 line
          g.drawString(
              slide.getCharacterRight().getText(),
              603 - (metrics.stringWidth(slide.getCharacterRight().getText()) / 2),
              250);
        } else {
          ...
          g.drawString(
              firstLine, 603 - (metrics.stringWidth(firstLine) / 2), 250 - metrics.getMaxDescent());
          g.drawString(
              secondLine,
              603 - (metrics.stringWidth(secondLine) / 2),
              250 + metrics.getMaxAscent());
        }
      }
  ...
  private void generateHTML(ArrayList<String> imageNames, String exportPath)
    ...
    for (String name : imageNames) {
      ...
      html.append("<td><center><img src=\"")
          .append(name)
          .append("\" width=\"500\" height=\"500\"></center></td>\n");
    ...
    for (SavedSlide slide : slideArrayList) {
      ...
      if (slide.getCharacterLeft() != null) {
        ...
        String skinHex =
            "#"
                + (format(skin.getRed())
                        + format(skin.getGreen())
                        + format(skin.getBlue())
                        + format(skin.getOpacity()))
                    .toUpperCase();
        String hairHex =
            "#"
                + (format(hair.getRed())
                        + format(hair.getGreen())
                        + format(hair.getBlue())
                        + format(hair.getOpacity()))
                    .toUpperCase();
  ...
  public boolean validDocument(Document document) {
    if (!document
        .getDocumentElement()
        .getNodeName()
        .equalsIgnoreCase("comic")) { // comic tag missing
      return false;
    }
  ...
  public boolean validPanel(Element panel, ArrayList<Character> poseList) {
    ...
    if (leftNode == null
        || rightNode == null
        || leftFigureNode == null
        || rightFigureNode == null
        || leftFigureNameNode == null
        || rightFigureNameNode == null) {
      return false;
    }
    ...
    Character left =
        poseList.stream()
            .filter(character -> leftName.getTextContent().equalsIgnoreCase(character.getName()))
            .findAny()
            .orElse(null);
    Character right =
        poseList.stream()
            .filter(character -> rightName.getTextContent().equalsIgnoreCase(character.getName()))
            .findAny()
            .orElse(null);
  ...
  public SavedSlide loadPanel(
      Element panel, int id, ArrayList<Character> poseList, ArrayList<Bubble> bubbleList) {
    ...
    c =
        poseList.stream()
            .filter(
                character -> leftFigureName.getTextContent().equalsIgnoreCase(character.getName()))
            .findAny()
            .orElse(null);
    ...
    if (leftBalloon != null && !leftBalloon.getAttribute("status").equals("")) {
      ...
      b =
          bubbleList.stream()
              .filter(
                  bubble -> leftBalloon.getAttribute("status").equalsIgnoreCase(bubble.getName()))
              .findAny()
              .orElse(null);
      if (b != null
          && leftBalloonContent != null
          && !leftBalloonContent.getTextContent().equals("")) {
    ...
    c =
        poseList.stream()
            .filter(
                character -> rightFigureName.getTextContent().equalsIgnoreCase(character.getName()))
            .findAny()
            .orElse(null);
    ...
    if (rightFigureAppearance != null
        && rightFigureAppearance.getTextContent().equalsIgnoreCase("MALE")) {
      right.setGender();
    }
    ...
    if (rightBalloon != null && !rightBalloon.getAttribute("status").equals("")) {
      ...
      b =
          bubbleList.stream()
              .filter(
                  bubble -> rightBalloon.getAttribute("status").equalsIgnoreCase(bubble.getName()))
              .findAny()
              .orElse(null);
      if (b != null
          && rightBalloonContent != null
          && !rightBalloonContent.getTextContent().equals("")) {
  ...
  public ArrayList<SavedSlide> loadXML(
      ArrayList<Character> poseList, ArrayList<Bubble> bubbleList) {
    FileChooser fileChooser = new FileChooser();
        ...
        if (validDocument(doc)) {
          ...
          if (!unloadedPanels.isEmpty()) {
            StringBuilder message =
                new StringBuilder(
                    "The following panels could not be loaded due to syntax problems: ");
